...
def tree(client: CMClient, path: str | None) -> None:
    ...
    click.echo(".")
    last.append(True)
    for p, last[-1] in _lookahead(client.get_productions()):
        click.echo(f"{_tree_prefix(last)}[{p.id}] {p.name}")
        last.append(True)
        for c, last[-1] in _lookahead(client.get_campaigns(p.id)):
            click.echo(f"{_tree_prefix(last)}[{c.id}] {c.name}")
            last.append(True)
            for s, last[-1] in _lookahead(client.get_steps(c.id)):
                click.echo(f"{_tree_prefix(last)}[{s.id}] {s.name}")
                last.append(True)
                for g, last[-1] in _lookahead(client.get_groups(s.id)):
                    click.echo(f"{_tree_prefix(last)}[{g.id}] {g.name}")
                last.pop()
            last.pop()
        last.pop()
    last.pop()
