...
class BpsScriptHandler(ScriptHandler):
    ...
    async def _check_htcondor_job(
        ...
        # FIXME the bps_submit_dir is dynamic (generated by BPS) and capturing
        # the value is critical for subsequent checks. That this is only
        # captured on successful checks is problematic and hinders some
        # recovery actions, especially those taken out-of-band. It is also
        # problematic that once set in the parent, the bps_submit_dir can't be
        # changed because of the update rules. This should be stored in a
        # mutable metadata dict instead.
