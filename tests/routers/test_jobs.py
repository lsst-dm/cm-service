...
async def test_job_routes(client: AsyncClient, api_version: str) -> None:
    ...
    # check get methods
    await check_get_methods(client, api_version, entry, "job", models_.Job)

    # check update methods
    await check_update_methods(client, api_version, entry, "job", models_.Job)
    ...
    parent = check_and_parse_response(response, models_.Group)

    response = await client.get(
        f"{config.asgi.route_prefix}/{api_version}/job/get/{entry.id}/errors",
    )
    check_errors = check_and_parse_response(response, list[models_.PipetaskError])
    ...
    update_model = models_.JobUpdate(
    ...
    rescue_job = check_and_parse_response(response, models_.Job)
    ...
    rescue_node_model = models_.NodeQuery(fullname=parent.fullname)
    ...
    check_jobs = check_and_parse_response(response, list[models_.Job])
