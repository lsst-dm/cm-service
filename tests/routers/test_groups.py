...
async def test_group_routes(client: AsyncClient, api_version: str) -> None:
    ...
    response = await client.get(f"{config.asgi.route_prefix}/{api_version}/group/list")
    groups = check_and_parse_response(response, list[models_.Group])
    entry = [group for group in groups if str(uuid_int) in group.name][0]

    # check get methods
    await check_get_methods(client, api_version, entry, "group", models_.Group)

    # check update methods
    await check_update_methods(client, api_version, entry, "group", models_.Group)
