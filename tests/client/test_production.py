...
def test_production_client(pyclient: CMClient) -> None:
    def run_test_production_client() -> None:
        ...
        create_tree(pyclient, LevelEnum.campaign, uuid_int)
        ...
        # assert check_get_by_name.id == entry.id,
        ...
        #         await db.Production.get_row_by_fullname(session,
        # entry.fullname)
        ...
        #     check_update = await db.Production.update_row(entry.id,
        #         name="foo")
        #     assert check_update.name == "foo", "name should be foo"

        #     check_update2 = await check_update.update_values(session,
        #         name="bar")
        ...
        #    assert len([c for c in check]) == 1,
        #        "length of children should be 2"

        # delete everything we just made in the session
        delete_all_productions(pyclient)

        # confirm cleanup
        productions = pyclient.productions.get_rows()
        assert len(productions) == 0

    anyio.run(run_test_production_client)
