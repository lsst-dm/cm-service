...
async def add_scripts(
    ...
    prep_script = await client.script.create(
        name="prepare",
    ...
    collect_script = await client.script.create(
        name="collect",
        parent_name=element.fullname,
        parent_level=element.level,
        spec_block_name="null_script",
    ...
    script_depend = await client.script_dependency.create(
...
async def create_tree(
    ...
    step_depend = await client.step_dependency.create(
...
async def delete_all_productions(
    ...
    productions = await client.production.get_rows()

    for prod_ in productions:
        await client.production.delete(prod_.id)
...
async def check_scripts(
    ...
    scripts = await sub_client.get_scripts(entry.id)
    ...
    prereq_0 = await client.scripts.check_prerequisites(scripts[0].id)
    assert prereq_0, "check_prerequisites is False for first script"

    prereq_1 = await client.scripts.check_prerequisites(scripts[1].id)
    assert not prereq_1, "check_prerequisites is True for second script"

    no_scripts = await sub_client.get_scripts(entry.id, script_name="bad")
    assert len(no_scripts) == 0, "get_scripts with bad script_name did not return []"

    all_scripts = await sub_client.get_all_scripts(entry.id)
    assert len(all_scripts) != 0, "get_all_scripts with failed"

    await client.scripts.update(scripts[1].id, superseded=True)
    scripts_check = await sub_client.get_scripts(entry.id)
    assert len(scripts_check) == 1, "Failed to ignore superseded script"

    scripts_check = await sub_client.get_scripts(entry.id, skip_superseded=False)
    assert len(scripts_check) == 2, "Failed to respect skip_superseded"

    await client.script.update(scripts[1].id, superseded=False)
    ...
    await client.script.update(scripts[0].id, status=StatusEnum.failed)

    check = await client.retry_script(entry.id, "prepare")
...
async def check_get_methods(
    ...
    check_getall_nonefound = await sub_client.get_rows(
        parent_name="bad",
        parent_class=parent_class,
    )
    assert len(check_getall_nonefound) == 0, "length should be 0"

    check_get = await sub_client.get_row(entry.id)
    ...
    check_get_by_name = await sub_client.get_row_by_name(name=entry.name)
    ...
    check_get_by_fullname = await sub_client.get_row_by_fullname(entry.fullname)
    assert check_get_by_fullname.id == entry.id, "pulled row should be identical"

    await sub_client.get_specification(entry.id)
    # assert specification.name == specification_check.name

    check = await sub_client.get_tasks(entry.id)
    assert len(check.reports) == 0, "length of tasks should be 0"

    check = await sub_client.get_wms_reports(entry.id)
    assert len(check.reports) == 0, "length of reports should be 0"

    check = await sub_client.get_products(entry.id)
    assert len(check.reports) == 0, "length of products should be 0"

    sleep_time = await sub_client.estimate_sleep_time(entry.id)
