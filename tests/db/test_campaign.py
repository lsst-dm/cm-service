...
async def test_campaign_db(engine: AsyncEngine) -> None:
    ...
    async with engine.begin():
        ...
        with pytest.raises(IntegrityError):
            await legacy.Campaign.create_row(
                session,
        ...
        with pytest.raises(errors.CMMissingRowCreateInputError):
            await legacy.Campaign.get_create_kwargs(
                session,
                name=f"camp0_{uuid_int}",
            )
        ...
        check_getall = await legacy.Campaign.get_rows(
            session,
        )
        ...
        with pytest.raises(errors.CMMissingRowCreateInputError):
            await legacy.Script.create_row(session)
        ...
        await check_get_methods(session, entry, legacy.Campaign, None)

        with pytest.raises(errors.CMMissingIDError):
            await legacy.Campaign.delete_row(session, -99)
        ...
        await check_update_methods(session, entry, legacy.Campaign)
