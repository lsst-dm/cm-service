...
async def test_job(engine: AsyncEngine) -> None:
    ...
    async with engine.begin():
        ...
        await interface.load_specification(session, "examples/example_standard_elements.yaml")
        await interface.load_specification(session, "examples/example_standard_scripts.yaml")
        ...
        await ScriptTemplate.load(
            session, "bps_core_yaml_template", f"{CM_CONFIGS}/templates/example_bps_core_yaml_template.yaml"
        )
        await ScriptTemplate.load(
            session,
        ...
        handler = Handler.get_handler(spec_block.id, handler_name)
