...
async def test_others_cli(uvicorn: UvicornProcess, api_version: str) -> None:
    ...
    check = check_and_parse_result(result, list[models_.PipetaskError])
    assert len(check) == 0

    result = runner.invoke(client_top, "product_set list --output yaml")
    products = check_and_parse_result(result, list[models_.ProductSet])
    assert len(products) == 0

    result = runner.invoke(client_top, "script_dependency list --output yaml")
    dependencies = check_and_parse_result(result, list[models_.Dependency])
    assert len(dependencies) == 0

    result = runner.invoke(client_top, "script_error list --output yaml")
    scripts = check_and_parse_result(result, list[models_.ScriptError])
    assert len(scripts) == 0

    result = runner.invoke(client_top, "task_set list --output yaml")
    tasks = check_and_parse_result(result, list[models_.TaskSet])
    assert len(tasks) == 0

    result = runner.invoke(client_top, "wms_task_report list --output yaml")
    reports = check_and_parse_result(result, list[models_.WmsTaskReport])
