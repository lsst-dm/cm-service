...
async def test_job_cli(runner: CliRunner) -> None:
    ...
    jobs = check_and_parse_result(result, list[models_.Job])
    ...
    # check get methods
    check_get_methods(runner, client_top, entry, "job", models_.Job)

    # check update methods
    check_update_methods(runner, client_top, entry, "job", models_.Job)
    ...
    parent = check_and_parse_result(result, models_.ElementMixin)

    result = runner.invoke(client_top, f"job get errors --row_id {entry.id} --output yaml")
    job_errors = check_and_parse_result(result, list[models_.PipetaskError])
    ...
    rescue_job = check_and_parse_result(result, models_.Job)
    ...
    check_accept = check_and_parse_result(result, models_.Job)
    ...
    rescue_jobs = check_and_parse_result(result, list[models_.Job])
