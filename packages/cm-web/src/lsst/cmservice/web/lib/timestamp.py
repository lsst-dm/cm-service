...
def timestamp(timestamp_: int, format: str) -> str:
    """Produce an arbitrary timestamp string using the provided format."""
    return utc_datetime(timestamp_).strftime(format)


def iso_timestamp(timestamp_: int) -> str:
    ...
    """
    return timestamp(timestamp_, format="%Y-%m-%dT%H:%M:%SZ")
