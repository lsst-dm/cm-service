...
from lsst.cmservice.api.handlers import interface
from lsst.cmservice.core import db
from lsst.cmservice.core.common import timestamp
from lsst.cmservice.core.common.daemon import daemon_iteration
...
async def test_daemon_db(engine: AsyncEngine) -> None:
    ...
    async with engine.begin():
        ...
        CM_CONFIGS = Path(__file__).parent.parent / "fixtures" / "examples"
        os.environ["CM_CONFIGS"] = str(CM_CONFIGS)
        ...
        queue_entry = await db.Queue.create_row(
            ...
            time_updated=timestamp.now_utc(),
            active=True,
        )
        ...
        await queue_entry.update_values(
            session,
            time_next_check=timestamp.now_utc(),
        )
